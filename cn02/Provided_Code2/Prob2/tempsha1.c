...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  result = SHA1_Init(&c); /* SHA1_Initにcのアドレスを渡して初期化. */
  /*
  if (result == 1){
...
result = SHA1_Update(&c, rbuff, readByte(argv[1])); /* SHA1_Updateでソースデータとしてargv[1]を追加. */
/*
if (result == 1){
...
for (i = 0; i < 20; i++){ /* SHA_DIGEST_LENGTHの分だけ繰り返す. */
